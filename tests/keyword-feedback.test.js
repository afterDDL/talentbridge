...
const checks = vm.runInContext(`
  ...
  ({
    ...
    reviewHasDecisionWorkflow: ["招聘推进到哪一步", "AI 找回效果", "提交判断并更新招聘进展"].every(text => reviewHtml.includes(text)),
    ...
    reviewCollapsesModelMetrics: reviewHtml.includes("review-metric-details") && reviewHtml.includes("查看模型指标"),
    reviewHasCandidateDecisionForms: reviewHtml.includes("comparison-candidate-tabs") && reviewHtml.includes("1. 提交 HR 决策") && reviewHtml.includes("2. 更新招聘进展"),
    reviewRemovedOldModules: !reviewHtml.includes("下一轮应该去哪里找人") && !reviewHtml.includes("review-advanced-details") && !reviewHtml.includes("当前 Demo 指标"),
    candidateFocusesOnEvidence: candidateHtml.includes("正在复核") && candidateHtml.includes("风险边界") && candidateHtml.includes("首轮验证问题") && !candidateHtml.includes("1. 提交 HR 决策"),
    reviewKeepsDecisionFormsAfterSourcingData: reviewResultHtml.includes("comparison-decision-panel") && !reviewResultHtml.includes("打开寻访策略"),
