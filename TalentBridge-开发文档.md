...
```http
POST /api/generate-sourcing-keywords
Content-Type: application/json
```

该接口根据“HR 推荐联系，且已进入联系、面试、Offer 或入职阶段”的候选人反向提炼下一轮主动寻访策略。前端会先在浏览器读取这些候选人的简历并执行隐私脱敏，再发送以下信息：
...
接口返回关键技术、产品或平台、相邻岗位、目标公司、排除词和可复制的布尔搜索式。目标公司只能来自正向样本的任职公司或已有研究证据支持的业务主体，不能由模型凭行业印象补充。
...
当“推荐联系”的候选人产生已联系或更深阶段的结果后，系统会自动更新“寻访关键词反哺”。样本按已入职、Offer、面试、联系的顺序优先使用，最多分析 8 份正向样本。该功能的目的是把已验证的人才特征转化为下一轮招聘网站搜索策略，而不是把 AI 自己给出的高分循环当作有效标签。
